Feature: Level
  ...
  Background:
    ...
    Scenario: I place a token (from the required tokens) on the board
      Given I place token 0 (from the required tokens) on the level's board at (0, 0)
      Then the token on the level's board at (0, 0) should be a Double Mirror token
  ...
  Scenario: Place all tokens and verify all tokens are placed
    Given I place token 0 (from the required tokens) on the level's board at (0, 0)
    And I place token 0 (from the required tokens) on the level's board at (1, 1)
